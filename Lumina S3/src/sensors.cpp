...
double bandPeak[NUM_FREQ_BANDS] = {0};
...
// ============================================================================
// AUDIO PROCESSING PIPELINE (esp-dsp SIMD)
// ============================================================================

void computeGlobalLevels() {
  bassLevel = 0;
  for (int b = 0; b < 4; b++) bassLevel += bandMagnitudes[b];
  bassLevel /= 4.0;
  midLevel = 0;
  for (int b = 4; b < 11; b++) midLevel += bandMagnitudes[b];
  midLevel /= 7.0;
  trebleLevel = 0;
  for (int b = 11; b < NUM_FREQ_BANDS; b++) trebleLevel += bandMagnitudes[b];
  trebleLevel /= 5.0;
  globalAudioLevel = 0;
  for (int b = 0; b < NUM_FREQ_BANDS; b++) globalAudioLevel += bandMagnitudes[b];
...
void audioProcessingTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    for (int b = 0; b < NUM_FREQ_BANDS; b++) {
      ...
      bandMagnitudes[b] = smoothedBandMagnitudes[b];
    ...
    if (environmentIsSilent) {
      for (int b = 0; b < NUM_FREQ_BANDS; b++) bandMagnitudes[b] = 0;
    } else {
      for (int b = 0; b < NUM_FREQ_BANDS; b++) {
        bandMagnitudes[b] = constrain(smoothedBandMagnitudes[b] / (bandPeak[b] * 1.2f), 0.0, 1.0);
      }
    }

    float bassSum = 0, midSum = 0, highSum = 0;
    for (int b = 0; b < NUM_FREQ_BANDS; b++) {
      if (b <= 3) bassSum += bandMagnitudes[b];
      else if (b <= 9) midSum += bandMagnitudes[b];
      else highSum += bandMagnitudes[b];
    }
